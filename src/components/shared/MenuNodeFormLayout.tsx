			...
			<Accordion type="single" collapsible className="w-full">
				<AccordionItem value="config">
					...
					<AccordionContent className="mt-2 space-y-3 bg-gray-50 p-3 dark:bg-gray-900/40">
						{(["onTrue", "onFalse", "onError"] as const).map((key) => (
							<NodeSelectAccordion
								key={key}
								title={
									key === "onTrue"
								...
								selectedId={data[key]}
								handleId={key}
								onSelect={(val) => updateNodeData(id, { [key]: val })}
								createConnection={(targetId) => createConnection(targetId, key)}
								removeConnection={(targetId) => removeConnection(targetId, key)}
								accentColor={`text-${colorAccent}-700 dark:text-${colorAccent}-300`}
							/>
						))}
					</AccordionContent>
			...
			<div className="flex flex-col gap-3">
				...
				{options.map((opt, index) => (
					<Accordion
						...
						value={expandedOptionIndex === index ? "open" : undefined}
						onValueChange={() =>
							setExpandedOptionIndex(
						...
						<AccordionItem value="open">
							...
							<AccordionContent className="space-y-2 px-3 py-3">
								...
								<NodeSelectAccordion
									...
									onSelect={(val) => handleUpdateOption(index, "next", val)}
									createConnection={(targetId) =>
										createConnection(targetId, `option-${index}`)
									}
									removeConnection={(targetId) =>
										removeConnection(targetId, `option-${index}`)
									}
									accentColor={`text-${colorAccent}-700 dark:text-${colorAccent}-300`}
								/>
